...
func genTestTx() (*bt.Tx, error) {
	...
	for i := 0; i < numTxins; i++ {
		randTxIn := &bt.Input{
			PreviousTxOutIndex: rand.Uint32(),
	...
	for i := 0; i < numTxouts; i++ {
		randTxOut := &bt.Output{
			Satoshis:      rand.Uint64(),
			LockingScript: bscript.NewFromBytes(make([]byte, rand.Intn(30))),
		}
		b := make([]byte, rand.Intn(30))
		if _, err := rand.Read(b); err != nil {
			return nil, err
		}
		randTxOut.LockingScript = bscript.NewFromBytes(b)
		tx.AddOutput(randTxOut)
	}
